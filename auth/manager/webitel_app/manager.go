...
import (
	...
	session "github.com/webitel/media-exporter/auth/session/user_session"
)
...
func (i *Manager) AuthorizeFromContext(ctx context.Context) (auth.Auther, error) {
	var token []string
	...
	return ConstructSessionFromUserInfo(sess.(*authmodel.Userinfo), getClientIP(ctx)), nil
}

func ConstructSessionFromUserInfo(userinfo *authmodel.Userinfo, ip string) *session.UserAuthSession {
